...
class KGActionDistNet(network.DistributionNetwork):
    ...
    def call(self,
        ...
        if observations.shape[1] == 1001:
            ...
            with_history = True
            history, question, actions = tf.split(observations, [1, 1, 1000],
        ...
        #print('Graph Embedding: ', graph_embedding_tensor)
        ...
        scores = tf.add(scores, graph_embedding_tensor)
        #print('Scores: ', scores)
        scores = tf.add(tf.math.scalar_mul(2.6,scores), tf.math.scalar_mul(0.4,graph_embedding_tensor))
